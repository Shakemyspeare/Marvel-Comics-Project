...
def results_view(request, data):
    ...
    for character in characters:
        ...
        character_urls = character['urls']
        urls = []
        i = 0
        for character_url in character_urls:
            hero_url = character_url['url']
            print(str(i) + ':' + hero_url)
            i += 1
            if hero_url[19] == 'h':
                urls.append(hero_url)
        characters_info = {'names': character['name'], 'descriptions': character['description'],
                           'paths': images['path'], 'extensions': images['extension'], 'ids': character['id']}
...
def random_view(request):
    params = {'limit': 1, 'offset': random.randrange(1, 1500), 'ts': timestamp, 'apikey': pub_key,
              'hash': hash_params()}
    res = requests.get('https://gateway.marvel.com:443/v1/public/characters', params=params)
    results = res.json()
    ...
    for random_character in random_characters:
        ...
        character_urls = random_character['urls']
        links = ''
        for character_url in character_urls:
            hero_url = character_url['url']
        ...
        characters_info = {'names': random_character['name'], 'descriptions': random_character['description'],
                           'paths': images['path'], 'extensions': images['extension'], 'hero_link': links,
                           'ids': random_character['id'], 'attriText': attributiontext}
